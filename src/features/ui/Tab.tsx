...
const ModalTab = ({ tabs }: PropsType) => {
  ...
  const [element, setElemet] = useState<JSX.Element>(tabs[0].element);
  let clicked = 0;

  //탭 클릭
  const recentStore = (e: React.MouseEvent<HTMLElement>) => {
    //클릭하면 clicked에 몇번째 애를 클릭했는지 알려줘야함.
    const index = Number(e.currentTarget.getAttribute("value"));
    clicked = index;
    setCurrentTab(index);
    {
      tabs.map((res, i) => (clicked === i ? setElemet(res.element) : null));
    }
  };
